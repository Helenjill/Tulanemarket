...
import { Loader2, ArrowRight, Check, X, Trash2 } from 'lucide-react';
...
export const Offers: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribeMade = onSnapshot(
      qMade,
      async (snapshot) => {
        const fetched = [];
      ...
      (error) => {
        console.error('Offers made query failed:', error);
        setLoading(false);
      }
    );
    ...
    const unsubscribeReceived = onSnapshot(
      ...
      (error) => {
        console.error('Offers received query failed:', error);
        setLoading(false);
      }
    );
  ...
  return (
    <div className="max-w-5xl mx-auto py-8">
      <h1 className="text-3xl font-bold text-text-primary mb-8 px-4 sm:px-0">
        Offers Dashboard
      </h1>
            ...
            {offersReceived.length === 0 ? (
              ...
              <div className="space-y-4">
                {offersReceived.map((offer) => (
                  <div
                    key={offer.id}
                    className={`border border-border-ink rounded-xl p-4 transition-colors ${
                      offer.status === 'pending'
                        ? 'bg-light-blue/20 border-accent-blue/40'
                        : 'bg-bg-muted'
                    }`}
                  >
                    <div className="flex justify-between items-start mb-3 gap-3">
                      ...
                      <div className="text-right">
                        <div className="font-bold text-xl text-tulane-green">
                          ${offer.amount}
                        </div>
                        <span
                    ...
                    {offer.status === 'pending' ? (
                      <div className="flex gap-2 mt-4 pt-4 border-t border-border-ink">
                        ...
                        <button
                          ...
                          className="px-3 bg-white text-red-600 border border-red-200 rounded-xl hover:bg-red-50 transition-colors"
                          title="Delete offer"
                        >
                      ...
                      <div className="flex justify-end mt-4 pt-4 border-t border-border-ink">
                        <button
                          onClick={() => handleDeleteOffer(offer.id, 'offer')}
                          className="px-3 py-2 bg-white text-red-600 border border-red-200 rounded-xl text-sm font-semibold flex items-center justify-center hover:bg-red-50 transition-colors"
                        >
                    ...
                    <div className="text-xs text-text-secondary mt-3">
                      {offer.createdAt
                        ? formatDistanceToNow(offer.createdAt.toDate()) + ' ago'
                        : ''}
                    </div>
                  </div>
                ))}
            ...
            {offersMade.length === 0 ? (
              ...
              <div className="space-y-4">
                {offersMade.map((offer) => (
                  <div
                    key={offer.id}
                    className="border border-border-ink rounded-xl p-4 bg-white relative"
                  >
                    <div className="flex justify-between items-start mb-2 gap-3">
                      <Link
                        to={`/listing/${offer.listingId}`}
                        className="font-semibold text-text-primary hover:text-accent-blue transition-colors flex items-center"
                      >
                        {offer.listing?.title || 'Unknown Listing'}
                    ...
                    <div className="flex justify-end mt-4 pt-4 border-t border-border-ink">
                      ...
                      >
                        <Trash2 className="w-4 h-4 mr-1" />
                        {offer.status === 'pending' ? 'Cancel Offer' : 'Delete'}
                      </button>
                    </div>
                  </div>
                ))}

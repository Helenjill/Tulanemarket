...
export const Home: React.FC = () => {
  ...
  const [favorites, setFavorites] = useState<{ [listingId: string]: string }>({});
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      snapshot.docs.forEach((docSnap) => {
        favMap[docSnap.data().listingId] = docSnap.id;
      });
  ...
  const filteredAndSortedListings = useMemo(() => {
    ...
    if (minPrice !== '') {
      result = result.filter((item) => Number(item.price) >= parseFloat(minPrice));
    }

    if (maxPrice !== '') {
      result = result.filter((item) => Number(item.price) <= parseFloat(maxPrice));
    }
    ...
    return result;
  }, [listings, selectedCategory, searchQuery, minPrice, maxPrice, selectedCondition, sortBy]);
      ...
      <div>
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-6 gap-4">
          <div>
            ...
            <div className="text-xs text-text-secondary mt-1">
              {loading ? (
                ...
                <span>
                  Showing {filteredAndSortedListings.length} item
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 text-[11px] font-bold uppercase tracking-wider border rounded-xl transition-colors ${
              showFilters
                ? 'bg-light-blue text-accent-blue border-accent-blue'
                : 'bg-white text-text-secondary border-border-ink hover:text-text-primary hover:bg-bg-muted'
            }`}
          >
        ...
        {loading ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-5">
            {[...Array(10)].map((_, i) => (
              <div
                key={i}
                className="animate-pulse flex flex-col space-y-2 border border-border-ink rounded-2xl overflow-hidden p-0"
              >
            ...
            {filteredAndSortedListings.map((listing) => {
              ...
              return (
                <Link
                  ...
                  className="group flex flex-col border border-border-ink rounded-2xl overflow-hidden bg-white relative hover:shadow-lg transition-all duration-300"
                >
                  <div className="relative aspect-square w-full bg-slate-100 overflow-hidden border-b border-border-ink flex items-center justify-center text-xs text-text-secondary">
                    ...
                    {listing.status === 'sold' && (
                      <div className="absolute inset-0 bg-black/60 flex items-center justify-center z-20 backdrop-blur-[2px]">
                        <span className="text-white font-extrabold tracking-widest uppercase text-xs">
                          Sold
                        </span>
                      </div>
                    )}
                  ...
                  <div className="p-4">
                    <div className="flex justify-between items-start mb-1 gap-2">
                      <div className="font-extrabold text-lg text-text-primary leading-none">
                        ${listing.price}
                      </div>
                    </div>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div className="text-[9px] text-text-secondary uppercase tracking-wider font-bold bg-bg-muted px-1.5 py-0.5 border border-border-ink rounded-md">
                        {listing.condition}
                      </div>
                    </div>
                  </div>
